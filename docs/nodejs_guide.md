...
However, in order to *create* those squished files, the frontend desperately needs heavy developer tools (like TypeScript, Vite, and ESLint). These tools are stored in `"devDependencies"`.
...
**In Short:**
* **Frontend (AWS S3)**: Needs a massive install to build, but uploads *nothing* from `node_modules`.
* **Backend (AWS Lambda)**: Needs a tiny install, because it uploads *everything* from `node_modules`.
