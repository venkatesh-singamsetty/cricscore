...
## 🛡️ v2.2.0: Enterprise Security & Dependency Upgrades [2026-06-16]
* **Security Scanning Tunnels**: Integrated Checkov (Terraform misconfiguration scans), Trivy (high/critical package filesystem scans), and GitHub CodeQL (SAST vulnerability code audits) directly into the pull request and deployment workflows.
* **Automated Dependency Auditing**: Configured Dependabot (`.github/dependabot.yml`) to automatically schedule and track daily package and provider upgrades.
* **Hardened CI/CD Branches**: Restructured push triggers to enforce that production deployments only trigger off pushes/merges to the `main` branch, isolating the live environment from experimental feature pushes.
* **Merged Package Upgrades**: Merged 5 dependency upgrades generated by Dependabot:
  - React to `19.2.7` (and manually aligned `react-dom` to `19.2.7` to prevent runtime version mismatch crashes).
  - `eslint-plugin-react-refresh` to `0.5.3`
  - `pg` driver to `8.21.0` (in `match-api` and `storage-worker`)
  - `@aws-sdk/client-sns` to `3.1069.0` (in `score-update`)
* **CI Concurrency Rules**: Added cancel-in-progress concurrency groups to optimize build pipeline efficiency.
