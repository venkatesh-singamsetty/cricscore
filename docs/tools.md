...
| Tool                          | Purpose                                                                                                                                                                                                                             |
| ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Husky & Lint-Staged**       | Native git hooks manager. Intercepts local `git commit` commands to run automated formatting (Prettier) and secret scanning (Gitleaks) strictly on staged files, preventing bad code or secrets from ever entering the git history. |
| **Prettier**                  | Opinionated code formatter that enforces a consistent style across the entire JavaScript, TypeScript, and JSON codebase automatically before commits.                                                                               |
| **Semantic Release**          | Automates the entire package release workflow including: determining the next version number, generating release notes, and publishing Git tags. It is driven purely by Conventional Commit messages (e.g. `feat:`, `fix:`).        |
| **Terraform Drift Detection** | A custom GitHub Action cron job that runs `terraform plan` nightly to detect and alert if anyone bypasses infrastructure-as-code by making manual changes in the AWS Console.                                                       |
