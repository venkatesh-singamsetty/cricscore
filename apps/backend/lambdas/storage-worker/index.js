...
exports.handler = async (event) => {
  ...
  try {
    ...
    for (const record of records) {
      ...
      console.log(
        `Processing event for match: ${matchId}, type: ${matchEvent.type}`,
      );
      ...
      if (undo) {
        ...
        if (lastBallRes.rows.length > 0) {
          ...
          const batterRunsToDeduct =
            !isWide && !isBye && !isLegBye ? ball.runs : 0;
          ...
          const isBowlerWicket =
            ball.is_wicket &&
            !["RUN_OUT", "RETIRED_HURT", "RETIRED_OUT"].includes(
              ball.wicket_type,
            );
          ...
          await client.query("DELETE FROM ball_events WHERE id = $1", [
            ball.id,
          ]);
        }
      }
      ...
      if (!syncOnly && ballData && !undo) {
        ...
        const batterRunsToAdd =
          !isWide && !isBye && !isLegBye ? ballData.runs : 0;
        ...
        const isBowlerWicket =
          ...
          ((!isNoBall &&
            ...
            (isWide &&
              ["STUMPED", "HIT_WICKET"].includes(ballData.wicketType)));
